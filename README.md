...
```
                    ┌─────────────────────────────────────────┐
   域名层（无证书）  │ ad-domain.list  28.6 万条域名           │  拦掉大部分广告
                    │ ad-domain-lite.list  6 万条（高置信）   │  怕误杀就用它
                    │ ad-rule.list  IP / 关键词 / 正则        │
                    └─────────────────────────────────────────┘
                                      ↓ 装完这层就够用很久
                    ┌─────────────────────────────────────────┐
   模块层（需证书）  │ ads-all.srmodule  改响应体 / 去开屏广告 │  管域名拦不住的
                    │ httpdns.srmodule  拦 HTTPDNS 绕行       │  必需配套
                    │ apps/*.srmodule   726 个 per-App 可选   │  出事时的逃生通道
                    │ mitm-exclude      排除解密的域名        │  银行类用这个
                    └─────────────────────────────────────────┘
                                      ↓
                    ┌─────────────────────────────────────────┐
   配置层（可选）    │ base.conf  代理分组 + 分流 + 引用上面两层│  自带骨架，节点自填
                    └─────────────────────────────────────────┘
```
